...
def call_Qexe(qexefilepath,queue):
	...
	if db == 'MYSQL':
		...
		port = dfCred.iat[4,5]
		try:
			connection = pyodbc.connect(driver=driver,server=server,trusted_connection='yes') #Not sure if object returned will be compatible with pandas read_sql function
	...
	if crossDB:
		...
		if crossDbSrc == 'MYSQL':
			...
			try:
				srcConnection = pyodbc.connect(driver=driver,server=server,trusted_connection='yes') #Not sure if object returned will be compatible with pandas read_sql function
		...
		if crossDbTgt == 'MYSQL':
			...
			try:
				tgtConnection = pyodbc.connect(driver=driver,server=server,trusted_connection='yes') #Not sure if object returned will be compatible with pandas read_sql function
				print 'Connected to SQL Server'
				log.write('Connected to SQL Server\n')
			except  Exception as e1:
	...
	for i,row in df.iterrows():
		log = open(r'SpartaRunTimeLog.txt','a')
	...
	statusgraph.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
...
def call_SpartaSQLServer(continuerunflag,defectflag,queue,testsetpath,testsetname,qcuser,qcpass,SQLServer):
	...
	if test_set_object.Name==testset_name:
		...
		for test_case in testList :
			...
			newItem.Name = 'Run '+ datetime.now().strftime("%m/%d/%Y %I:%M:%S %p")
			...
			for step in steps :
				...
				if not text == "" :
					try:
						con = pyodbc.connect(driver='{SQL Server}',server=SQLServer,trusted_connection='yes')
					except Exception as e1:
						connflag = True
						print e1
						print '***ERROR ----- Connection to SQLServer Failed CHECK YOUR DB CREDENTIALS ----------ERROR ************'
						break
					cur = con.cursor()
					...
					if '_DATACOMP' in tcname:
						...
						try:
							...
							print '***SQL SERVER SYNTAX ERROR :\n\t {0}\t {1}'.format(errorcode,error)
							syntaxflag = True
							continue
						...
						res_rowcount = len(records_returned)
						if os.path.exists('LOGS')==False:
							os.mkdir('LOGS')
						printable = set(string.printable)
						tname = filter(lambda x: x in printable, test_case.Name)
						writ = pd.ExcelWriter('LOGS/'+tname+'_LOG.xlsx')
						try:
							pd.read_sql(sql=text, con=con) \
							.to_excel(writ,'OUTPUT', index=False)
							writ.save()
						except pyodbc.Error as e:
							errorcode = e.args[0]
							error = e.args[1]
						...
						# step.SetField("ST_ACTUAL", "Records Returned :"+str(res_rowcount)) # Wohhooooo!!!!
						if res_rowcount == 2:
						...
						#for data test cases 
						if res_rowcount == 0 and '_DATA' in tcname  :
							step.Status = "Passed"
							step.SetField("ST_ACTUAL", "Data match between SRC and TGT by minus query. Records Returned :"+str(res_rowcount)) # Wohhooooo!!!!
							passcount = passcount +1
						# for count minus test cases
						elif res_rowcount == 0 and '_COUNT' in tcname  :
							step.Status = "Passed"
							step.SetField("ST_ACTUAL", "Count match between SRC and TGT tables by minus query. Records Returned :"+str(res_rowcount)) # Wohhooooo!!!!
							passcount = passcount +1

						#for count test cases
						elif (cantdocount == False and '_COUNT' in tcname and res_rowcount == 3 ) :
						...
						elif (cantdocount == False and res_rowcount == 2 and srccount-tgtcount==0) :
							step.Status = "Passed"
							step.SetField("ST_ACTUAL", "Counts match between SRC and TGT tables by union all query. Matching Record Count: "+str(srccount)) # Wohhooooo!!!!
							passcount = passcount +1
						...
						elif res_rowcount >= 1 and '_COUNT' in tcname  :
							step.Status = "Failed"
							step.SetField("ST_ACTUAL", "Counts don't match between SRC and TGT tables.")
							failcount = failcount +1
							fail_flag = True
						# for DDL/Duplicate/other queries returning 0 records
							...
							step.Status = "Passed"
							step.SetField("ST_ACTUAL", "Data is matching. Records Returned :"+str(res_rowcount)) # Wohhooooo!!!!
							passcount = passcount +1

						#for failing data test cases where rowcount >= 1
						elif (cantdocount == True and res_rowcount >= 1) :
							step.Status = "Failed"
							step.SetField("ST_ACTUAL", "Data mismatch between SRC and TGT tables. Mismatching Record Count :"+str(res_rowcount))
							failcount = failcount +1
							fail_flag = True

						else :
							step.Status = "Failed"
							step.SetField("ST_ACTUAL", "Data Mismatch. Records Returned :"+str(res_rowcount))
					...
					if defectflag and fail_flag == True:
						for result in records_returned:
							...
							step.SetField("ST_ACTUAL", "Rows returned :"+str(res_rowcount) ) # Wohhooooo!!!!
							# step.SetField("ST_ACTUAL", "Rows returned :"+str(res_rowcount) + "Records :<br>" +result_string ) # Wohhooooo!!!!
						...
						bug.SetField("BG_SUMMARY", "Test Case : "+test_case.Name+" failed with records returend count :"+str(res_rowcount))
					...
					elif defectflag == False and fail_flag == True:
